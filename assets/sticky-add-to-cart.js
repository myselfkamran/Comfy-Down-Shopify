...
class VariantStickyAddToCart extends HTMLElement {
    ...
    updateAttribute(disable = true){
        ...
        if (disable) {
          ...
          try{
          stickyPrice.removeClass("wpd-processed");
          stickyPrice.attr("data-wpd-variant-compare-at-price",this.currentVariant.compare_at_price);
          stickyPrice.attr("data-wpd-variant-price",this.currentVariant.price);
          stickyPrice.attr("data-wpd-variant-id",this.currentVariant.id);
          // stickyPrice.attr("data-wpd-product-handle",this.product.handle);
        }catch(e){}
